...
def details(request, slug):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # pegar o valor de um atributo do form
...
def enrollment(request, slug):
    ...
    if created:
        ...
        messages.info(request, 'Você já está no curso')
...
def undo_enrollment(request, slug):
    ...
    if request.method == 'POST':
        enrollment.delete()
        messages.success(request, 'Inscrição do curso cancelada com sucesso')
        return redirect('accounts:dashboard')
...
def show_announcement(request, slug, pk):
    ...
    if form.is_valid():
        form.save(user=request.user, announcement=announcement)
        form = CommentForm()
...
def material(request, slug, pk):
    ...
    if not request.user.is_staff and not lesson.is_available():
        ...
        return redirect('courses:lesson', slug=slug, pk=lesson.pk)
